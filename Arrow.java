...
public class Arrow {
    ...
    private float height_Co = 0.8f;  //圆锥高度
    private float radius_Co = 0.3f;  //圆锥底面半径
    private float height_Cy = -1.0f;  //圆柱高度
    private float radius_Cy = 0.15f;  //圆柱底面半径
    ...
    public void draw(GL10 gl) {
        gl.glRotatef(angleX_Co, 1, 0, 0);
        gl.glRotatef(angleY_Co, 0, 1, 0);
        gl.glRotatef(angleZ_Co, 0, 0, 1);

        gl.glRotatef(angleX_Cy, 1, 0, 0);
        gl.glRotatef(angleY_Cy, 0, 1, 0);
        gl.glRotatef(angleZ_Cy, 0, 0, 1);
        ...
        gl.glColor4f(0.0f, 1.0f, 0.0f, 0.5f);//change the color
    }
}
